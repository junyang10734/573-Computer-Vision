...
def convolve2d(img, kernel):
    ...
    new_img = [[0 for x in range(len(img))] for y in range(len(img))]
    kernel_h = len(kernel)
    ...
    return new_img
    # return utils.crop(new_img, 0, len(new_img)-1, 1, len(new_img)-1)
...
def main():
    ...
    write_image(filtered_img, os.path.join(args.rs_dir, "{}.jpg".format(args.filter+'-dy')))
